...
void mod_reverb_effekt( fp_sample_t *in, fp_sample_t *out)
{
  ...
  const float re_gain = -0.05;
  const uint16_t delay = 1500;

  fp_sample_t fp_sample;
  sample_buffer_get_z( &fp_sample, delay);

  out->left_fp32 = in->left_fp32 + (fp_sample.left_fp32 * in_gain);
  out->right_fp32 = in->right_fp32 + (fp_sample.right_fp32 * in_gain);

  fp_sample.left_fp32 = (out->left_fp32 * re_gain);
  fp_sample.right_fp32 = (out->right_fp32 * re_gain);
